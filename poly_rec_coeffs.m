function AB = poly_rec_coeffs(n_max, polytype,pdfName, parameters, bounds)
...
if strcmpi(polytype,'arbitrary')
    CDFquantiles = all_icdf([ .001, .1, .5, .9, .999]',pdfName,parameters,bounds);
    pdfFun = @(x)all_pdf(x,pdfName,parameters,bounds);
    AB = arbitrary_ml_integrator(n_max, pdfFun, bounds,CDFquantiles);
    return;
end
...
AB = [an(0:n_max)' , [sqrt_b0, sqrt_bn(1:(n_max))]'];
